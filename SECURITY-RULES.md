...
| 3 | **Prompt injection** — un sitio con texto tipo "ignora tus instrucciones" para manipular a un LLM | Cubierto por diseño: el motor es DETERMINISTA (curl/grep + mediciones del navegador, sin IA). El informe solo contiene textos predefinidos + títulos de una lista fija; NUNCA se vuelca contenido crudo del sitio. Los dos datos del sitio que llegan al texto se sanitizan: X-Powered-By (`tr -cd`, 40 chars) y la `evidencia` de los hallazgos de `ux-audit` (`sanitizar()` en `scripts/ux-audit/lib.mjs`: lista blanca de caracteres, sin saltos de línea, 120 chars). |
